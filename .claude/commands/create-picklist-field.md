# Create Picklist Field for Salesforce Lead Object

## Overview
This instruction defines the process for creating a new picklist (dropdown) field for the Lead object in Salesforce using the standard metadata format.

## Requirements
1. Create a new picklist field for the Lead object
...
2. **Field Label**: Should match the field name without the `__c` suffix
3. **Required**: Set to `false` by default (can be changed if needed)
4. **Type**: Must be `Picklist`
5. **Value Set**: Must include at least one picklist value
6. **Value Structure**: Each value must have:
   - `fullName`: The internal value name
   - `default`: Set to `false` for all values (only one can be default)
   - `label`: The display label for the value

## Example Implementation
For a field named "Status" with values "New", "Contacted", and "Converted", the resulting file should be:
...
## Sample Usage
To create a picklist field named "Priority" with values "High", "Medium", and "Low":

1. Create the file: `force-app/main/default/objects/Lead/fields/Priority__c.field-meta.xml`
